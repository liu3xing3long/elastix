...
//------------------------------------------------------------------------------
std::size_t
OpenCLKernelManager::CreateKernel( const OpenCLProgram &program, const std::string &name )
{
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLKernelManager::BuildProgramFromSourceCode( const std::string &sourceCode, const std::string &prefixSourceCode,
                                                 ...
                                                 const std::string &extraBuildOptions )
{
    ...
    OpenCLProgram program = this->m_Context->BuildProgramFromSourceCode( devices, sourceCode, prefixSourceCode, postfixSourceCode, extraBuildOptions );
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLKernelManager::BuildProgramFromSourceFile( const std::string &fileName, const std::string &prefixSourceCode,
                                                 const std::string &postfixSourceCode,
                                                 const std::string &extraBuildOptions )
{
    const std::list< OpenCLDevice > devices = this->m_Context->GetDevices();
    OpenCLProgram program = this->m_Context->BuildProgramFromSourceFile( devices, fileName, prefixSourceCode, postfixSourceCode, extraBuildOptions );
...
//------------------------------------------------------------------------------
bool
OpenCLKernelManager::SetKernelArg( const std::size_t kernelId, const cl_uint argId, const std::size_t argSize,
                                   const void *argVal )
{
    ...
    {
        itkWarningMacro(
                "Setting kernel argument failed with GPUKernelManager::SetKernelArg(" << kernelId << ", " << argId
                                                                                      << ", " << argSize << ". "
                                                                                      << argVal << ")" );
    }
...
//------------------------------------------------------------------------------
bool
OpenCLKernelManager::SetKernelArgForAllKernels( const cl_uint argId, const std::size_t argSize, const void *argVal )
{
...
//------------------------------------------------------------------------------
bool
OpenCLKernelManager::SetKernelArgWithImage( const std::size_t kernelId, const cl_uint argId,
                                            const GPUDataManager::Pointer manager )
{
    if ( kernelId < 0 || kernelId >= this->m_Kernels.size() )
    {
        return false;
    }
    ...
    {
        error = clSetKernelArg( this->GetKernel( kernelId ).GetKernelId(), argId, sizeof( cl_mem ), manager->GetGPUBufferPointer() );
    }
    else
    {
    ...
    {
        itkWarningMacro(
                "Setting kernel argument failed with GPUKernelManager::SetKernelArgWithImage(" << kernelId << ", "
                                                                                               << argId << ", "
                                                                                               << manager << ")" );
    }
...
//------------------------------------------------------------------------------
OpenCLEvent
OpenCLKernelManager::LaunchKernel( const std::size_t kernelId, const OpenCLSize &global_work_size,
                                   const OpenCLSize &local_work_size, const OpenCLSize &global_work_offset )
{
    if ( kernelId >= this->m_Kernels.size() )
    {
        return OpenCLEvent();
...
//------------------------------------------------------------------------------
OpenCLEvent
OpenCLKernelManager::LaunchKernel( const std::size_t kernelId, const OpenCLEventList &event_list )
{
    if ( kernelId >= this->m_Kernels.size() )
    {
        return OpenCLEvent();
...
OpenCLEvent
OpenCLKernelManager::LaunchKernel( const std::size_t kernelId, const OpenCLEventList &event_list,
                                   const OpenCLSize &global_work_size, const OpenCLSize &local_work_size,
