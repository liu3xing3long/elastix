...
public:
    ...
    OpenCLContextPimpl(): id( 0 ), is_created( false ), last_error( CL_SUCCESS )
    {
    }
...
//------------------------------------------------------------------------------
OpenCLContext::OpenCLContext(): d_ptr( new OpenCLContextPimpl() )
{
}
            ...
            else
            {
                ...
                else
                {
                    itkGenericExceptionMacro(
                            << "Unable to create OpenCLContext with method MultipleMaximumFlopsDevices." );
                }
            }
        }
    }
...
OpenCLContext::CreateContext( const std::list< OpenCLDevice > &devices, OpenCLContextPimpl *d )
{
    ...
    if ( !devices.empty() )
    {
        ...
        cl_context_properties props[] = {CL_CONTEXT_PLATFORM,
                                         cl_context_properties( devices.front().GetPlatform().GetPlatformId() ), 0};
        d->id = clCreateContext( props, devs.size(), &devs[0], opencl_context_notify, 0, &(d->last_error) );
    }
    else
    {
...
//------------------------------------------------------------------------------
void
OpenCLContext::CreateContext( const OpenCLPlatform &platform, const OpenCLDevice::DeviceType type,
...
{
    static const char *errorString[] = {"CL_SUCCESS",                                   // 0
...
//------------------------------------------------------------------------------
OpenCLImage
OpenCLContext::CreateImageDevice( const OpenCLImageFormat &format, const OpenCLMemoryObject::Access access,
...
//------------------------------------------------------------------------------
OpenCLImage
OpenCLContext::CreateImageHost( const OpenCLImageFormat &format, void *data, const OpenCLSize &size,
...
//------------------------------------------------------------------------------
OpenCLImage
OpenCLContext::CreateImageCopy( const OpenCLImageFormat &format, const void *data, const OpenCLSize &size,
                                const OpenCLMemoryObject::Access access )
{
    if ( size.IsZero() )
    {
    ...
    cl_image_desc imageDesc;
    OpenCLImage::SetImageDescription( imageDesc, format, size );
    cl_mem mem = clCreateImage( d->id, flags, &(format.m_Format), &imageDesc, const_cast< void * >( data ),
                                &(d->last_error) );
#else
    cl_mem mem = NULL;
    if( size.GetDimension() == 1 )
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::CreateProgramFromSourceCode( const std::string &sourceCode, const std::string &prefixSourceCode,
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::CreateProgramFromSourceFile( const std::string &filename, const std::string &prefixSourceCode,
                                            const std::string &postfixSourceCode )
{
    if ( filename.empty() )
    {
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::CreateOpenCLProgram( const std::string &filename, const std::string &source,
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::BuildProgramFromSourceCode( const std::string &sourceCode, const std::string &prefixSourceCode,
                                           const std::string &postfixSourceCode )
{
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::BuildProgramFromSourceCode( const std::list< OpenCLDevice > &devices, const std::string &sourceCode,
                                           const std::string &prefixSourceCode, const std::string &postfixSourceCode,
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::BuildProgramFromSourceFile( const std::string &filename, const std::string &prefixSourceCode,
                                           const std::string &postfixSourceCode )
{
...
//------------------------------------------------------------------------------
OpenCLProgram
OpenCLContext::BuildProgramFromSourceFile( const std::list< OpenCLDevice > &devices, const std::string &fileName,
                                           const std::string &prefixSourceCode, const std::string &postfixSourceCode,
                                           const std::string &extraBuildOptions )
{
    OpenCLProgram program = this->CreateProgramFromSourceFile( fileName, prefixSourceCode, postfixSourceCode );
...
open_cl_get_supported_image_formats( const cl_context ctx, const cl_mem_flags flags,
...
//------------------------------------------------------------------------------
OpenCLSampler
OpenCLContext::CreateSampler( const bool normalizedCoordinates, const OpenCLSampler::AddressingMode addressingMode,
                              const OpenCLSampler::FilterMode filterMode )
{
    ITK_OPENCL_D( OpenCLContext );
    cl_sampler sampler = clCreateSampler( d->id, normalizedCoordinates ? CL_TRUE : CL_FALSE,
...
#ifdef CL_VERSION_1_2
    const cl_int error = clEnqueueMarkerWithWaitList( this->GetActiveQueue(), event_list.GetSize(),
                                                      event_list.GetEventData(), &event );
#else
...
#ifdef CL_VERSION_1_2
    cl_event event;
    const cl_int error = clEnqueueBarrierWithWaitList( this->GetActiveQueue(), event_list.GetSize(),
                                                       event_list.GetEventData(), &event );
#else
