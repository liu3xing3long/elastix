...
GPUDenseFiniteDifferenceImageFilter< TInputImage, TOutputImage, TParentImageFilter >::GPUDenseFiniteDifferenceImageFilter()
{
    /**
    ...
    // create kernel
    m_ApplyUpdateGPUKernelHandle = this->m_GPUKernelManager->CreateKernel( program, "ApplyUpdate" );
}
...
template< typename TInputImage, typename TOutputImage, typename TParentImageFilter >
void
GPUDenseFiniteDifferenceImageFilter< TInputImage, TOutputImage, TParentImageFilter >::GPUApplyUpdate( const TimeStepType &dt )
{
    ...
    // launch kernel
//  OpenCLEvent event = this->m_GPUKernelManager->LaunchKernel(m_ApplyUpdateGPUKernelHandle, (int)TInputImage::ImageDimension, globalSize,
//                                         localSize );
    
    OpenCLEvent event = this->m_GPUKernelManager->LaunchKernel( m_ApplyUpdateGPUKernelHandle, /*(int)TInputImage::ImageDimension,*/
                                                                OpenCLSize( globalSize ), OpenCLSize( localSize ) );
    
    event.WaitForFinished();
    
    itkDebugMacro( << "GPUDenseFiniteDifferenceImageFilter::GPUApplyUpdate() finished" );
