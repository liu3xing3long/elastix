 ...
 *
 *=========================================================================*/
#ifndef itkGPUThresholdImageFilter_hxx
#define itkGPUThresholdImageFilter_hxx
...
GPUThresholdImageFilter< TInputImage >
::GPUThresholdImageFilter()
{
  ...
  {
    itkExceptionMacro("GPUThresholdImageFilter supports 1/2/3D image.");
  }
...
//     }
  ...
  defines << "#define InArgType ";
  GetTypenameInString( typeid ( typename TInputImage::PixelType ), defines );
  defines << "#define OutArgType ";
  GetTypenameInString( typeid ( typename TInputImage::PixelType ), defines );

  itkDebugMacro( << "Defines: " << defines.str() << std::endl );
  const char* GPUSource = GPUThresholdImageFilter::GetOpenCLSource();
  ...
  this->m_UnaryFunctorImageFilterGPUKernelHandle = this->m_GPUKernelManager->CreateKernel(program, "ThresholdFilter");
}
...
::GPUGenerateData()
{
  ...
  InputPixelType lowerThreshold = this->GetLower();
  InputPixelType upperThreshold = this->GetUpper();
  ...
  this->GetFunctor().SetLowerThreshold( lowerThreshold );
  this->GetFunctor().SetUpperThreshold( upperThreshold );
