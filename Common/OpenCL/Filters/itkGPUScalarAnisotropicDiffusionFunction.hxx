 ...
 *
 *=========================================================================*/
#ifndef itkCLGPUScalarAnisotropicDiffusionFunction_hxx
#define itkCLGPUScalarAnisotropicDiffusionFunction_hxx
...
::GPUScalarAnisotropicDiffusionFunction()
{
  ...
  this->m_AnisotropicDiffusionFunctionGPUKernelManager = OpenCLKernelManager::New();
  ...
  const OpenCLProgram program =  this->m_AnisotropicDiffusionFunctionGPUKernelManager->BuildProgramFromSourceCode( GPUSource, defines.str().c_str() );

  // create kernel
  this->m_AverageGradientMagnitudeSquaredGPUKernelHandle =
    this->m_AnisotropicDiffusionFunctionGPUKernelManager->CreateKernel(program, "AverageGradientMagnitudeSquared");
}
...
::GPUCalculateAverageGradientMagnitudeSquared(TImage *ip)
{
  ...
  typename GPUImageType::SizeType localSize, globalSize;
  ...
  {
#ifdef GPU_DIFF_DEBUG
    std::cout << "initializing m_AnisotropicDiffusionFunctionGPUBuffer" << std::endl;
#endif
  ...
  typename OpenCLKernelManager::Pointer kernelManager = this->m_AnisotropicDiffusionFunctionGPUKernelManager;
...
#ifdef GPU_DIFF_DEBUG
  std::cout << "reading back done, sum " << sum << std::endl;
#endif
