...
GPURecursiveGaussianImageFilter< TInputImage, TOutputImage >::GPUGenerateData()
{
    ...
    {
        itkExceptionMacro( << "GPURecursiveGaussianImageFilter unable to perform. Device Memory out of range" );
        return;
    }
    ...
    if ( ImageDim == 3 )
    {
        ...
        switch ( this->GetDirection() )
        {
            ...
            case 2:
                ...
                break;
            default:
            itkExceptionMacro( << "GPURecursiveGaussianImageFilter unrecogniszed direction: " << this->GetDirection() );
                break;
        }
    }
    else if ( ImageDim == 2 )
...
#ifdef FILTER_DEBUG
    char debug_msg[4096];
    sprintf(debug_msg, "image dim: %d, imagesize: (%d, %d, %d), direction: %d \n", ImageDim,
            imgSize[0], imgSize[1], imgSize[2], this->GetDirection());
    std::cout << debug_msg;
#endif
    ...
    switch ( ImageDim )
    {
        ...
        case 3:
        {
            OpenCLEvent event = this->m_GPUKernelManager->LaunchKernel( m_FilterGPUKernelHandle, OpenCLSize( globalSize2D[0], globalSize2D[1] ) );
            event.WaitForFinished();
        }
            break;
    }
...
#ifdef FILTER_DEBUG
    sprintf(debug_msg, "%s \n", "GPURecursiveGaussianImageFilter::GPUGenerateData() done, all done");
    std::cout << debug_msg;
#endif
